...
@section('body')
    ...
    <div class="col-md-10 mx-auto col-lg-5">
        <form method="POST" action="/products/update/{{ $product->id }}" class='p-4 p-md-5 border rounded-3 bg-body-tertiary'
            id='meuFormulario'>
            @csrf
            ...
            <div>
                @if ($product->status == 1)
                    <div class="form-check">
                        <input class="form-check-input" type="radio" name="status" value="1" id="statusA" checked>
                        <label class="form-check-label" for="statusA">
                            Active
                        </label>
                    </div>
                    <div class="form-check">
                        <input class="form-check-input" type="radio" name="status" value="0" id="statusD">
                ...
                @else
                    ...
                    <div class="form-check">
                        <input class="form-check-input" type="radio" name="status" id="statusD" Value="0" checked>
                        <label class="form-check-label" for="statusD">
                            Disabled
                        </label>
                    </div>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
                    $("#btnupdate").click(function() {
                        $("#meuFormulario").attr("action", "/category/update/{{ $product->id }}");
                    });
    </script>
@endsection
